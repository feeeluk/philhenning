<?php
	...
	if (isset($_GET['manufacturer']))
		{
			...
			$description_query = "SELECT *
								FROM ecom_manufacturer
	         					WHERE ecom_manufacturer.manufacturer = '".$manufacturer."'
	         					LIMIT 1";								
			...
			$query = "SELECT ecom_product.id AS product_id, ecom_product.name AS product_name, price, year, ecom_product.description AS product_description, ecom_product.manufacturer_id AS manufacturer_id, manufacturer, image, ecom_type.name AS type_name 
					FROM ecom_product
	...
	// if the page has NOT been passed a specific type variable then do the following
		...
		{
			$query = "SELECT * FROM ecom_type ORDER BY name";
