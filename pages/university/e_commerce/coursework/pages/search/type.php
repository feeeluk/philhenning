	...
	if (isset($_GET['type']))
		{
			...
			$description_query = "SELECT description 
	         					FROM ecom_type
	         					WHERE ecom_type.name = '".$type."'
	         					LIMIT 1";

			// main query - all products from specific product type
			$query = "SELECT ecom_product.id AS product_id, ecom_product.name AS product_name, ecom_product.manufacturer_id AS manufacturer_id,year, price, ecom_product.description AS product_description, manufacturer, image, ecom_type.name AS type_name
					FROM ecom_product
