<?php
	...
	$page_title = "Edit user: ".$_GET['user'];
		...
		if (mysqli_num_rows($result) > 0)
		{
			while($row = mysqli_fetch_array($result, MYSQL_ASSOC))
			{
				if($row['is_admin'] == 1)
				{
					...
					$query = "SELECT * FROM ecom_user WHERE username = '".$usern."' ";
					...
					if(isset($_GET['delete']))
						{
							// delete image query
							$delete_query = "UPDATE ecom_user
											SET
											image = 'images/users/placeholder.png'
											WHERE username = '".$usern."'";
							
							mysqli_query($connection, $delete_query);
							echo "<script>window.location.assign('edit_user.php?user=".$usern."');</script>";
						}	
						...
						// form submit validation
							...
							if ($not_empty_password == true AND $password_match == true AND $not_empty_first_name == true AND $not_empty_last_name == true AND $not_empty_email == true AND $valid_email == true AND $valid_date == true)
								...
								if(is_uploaded_file($_FILES['image']['tmp_name']))
									...
									echo "<script type=\"text/javascript\">document.location.href=\"manage_users.php\";</script>";

								}	
						}		
					echo "<img class='main' src='".$path."assets/".$image."' />";
				?>
					<form action="edit_user.php?user=<?php echo $usern; ?>" method="post" enctype="multipart/form-data">
						<p>
						...
						<p>
							<span class="xxs">&nbsp;</span><a href='edit_user.php?user=<?php echo $usern; ?>&delete=yes' name='delete_image' id='delete_image'>delete image</a>
						</p>		
						...
						if($admin == 0)
							{
								echo "<p>
									...
									<p>
									<span class='xxs'>&nbsp;</span>
									Password = lecturer's first name
									</p>";
							}
						?>	
						...
						<p>
						<span class="xxs">Retype password</span>
						...
						<p>
						<span class="xxs">&nbsp;</span><input class="submit" type="submit" id="submit" name="submit" value="update">
