...
if(isset($_POST['submit']))
	...
	// assign form input values to the corresponding variables. 'selected' variables are used to store the currently selected option of drop down fields, so that the selected option will be maintained when the form is refreshed
		$product_id = mysqli_real_escape_string($connection, strip_tags(strtolower(trim($_POST['product_id']))));
		...
		$query = "SELECT id FROM ecom_product WHERE id = '$product_id'";
		...
		if ($not_empty_product_id == true AND $new_product_id == true AND $not_empty_name == true AND $not_empty_manufacturer == true AND $not_empty_type == true AND $not_empty_price == true AND $not_empty_year == true AND $valid_year == true AND $not_empty_description == true)
		{
			...
			// insert update query
			$query = "INSERT INTO ecom_product (id, name, manufacturer_id, type_id, price, year, description)
					SELECT '$product_id' AS product_id, '$name' AS name, ecom_manufacturer.id AS manufacturer_id, ecom_type.id AS type_id, '$price' AS price, '$year' AS year, '$description' AS description
					FROM ecom_manufacturer, ecom_type
					WHERE ecom_manufacturer.manufacturer = '$selected_manufacturer' AND ecom_type.name = '$selected_type'";
