		...
		if (mysqli_num_rows($result) > 0)
		{
			while($row = mysqli_fetch_array($result, MYSQL_ASSOC))
			{
				if($row['is_admin'] == 1)
				{
					...
					$not_empty_quantity = false;

					// variables that store error messages
					$required = "<span class='error'>Required</span>";					

					$product_query = "SELECT ecom_product.name AS product_name, ecom_manufacturer.id AS manufacturer_id, ecom_manufacturer.manufacturer AS manufacturer, ecom_type.id AS type_id, ecom_type.name AS type_name, price, year, ecom_product.description AS description, ecom_product.qty AS quantity
					...
					if(isset($_GET['delete']))
						{
							...
							// delete image query
							$delete_query = "DELETE FROM ecom_product_image
											WHERE  id = '".$image_id."'";
					...
					if(isset($_POST['submit']))
					{
						...
						// check if the quantity field contains a value
						if($quantity !="")
						{
							$not_empty_quantity = true;
						}

						// form submit validation	
						if ($not_empty_name == true AND $not_empty_manufacturer == true AND $not_empty_type == true AND $not_empty_price == true AND $not_empty_year == true AND $not_empty_description == true AND $not_empty_quantity == true)
						{
							// form submitted successfully

							// insert update query
							$query = 
									"UPDATE ecom_product AS product
									SET
									name = '$name',
									product.manufacturer_id = (
									   SELECT manufacturer.id
									   FROM ecom_manufacturer AS manufacturer
									   WHERE manufacturer.manufacturer = '$selected_manufacturer'
									),
									product.type_id = (
									   SELECT type.id
									   FROM ecom_type AS type
							...
							// UPLOAD PRODUCT IMAGES TO SERVER
								...
								function upload_image($x)
								{
									...
									if(is_uploaded_file($_FILES[$x]['tmp_name']))
										...
										$insert_query = "INSERT INTO ecom_product_image (product_id, location)
					...
					$image_query = "SELECT name, location, ecom_product_image.id AS image_id
					...
					<input type="hidden" name="product" id="product" value="<?php echo $product_id; ?>" />
						...
						<span class="xxs">Manufacturer</span>
							<select name="manufacturer" id="manufacturer">
								<?php
									...
									$query = "SELECT id, manufacturer FROM ecom_manufacturer";
									$result = mysqli_query($connection, $query);
									if(mysqli_num_rows($result) > 0)
									{
						...
						<span class="xxs">Type</span>
							<select name="type" id="type">
								<?php
									// pull the values for the drop down menu out of the corresponding database table
									$query = "SELECT id, name FROM ecom_type";
									$result = mysqli_query($connection, $query);
									if(mysqli_num_rows($result) > 0)
									{
										// Set the default drop down option unless already selected
