				...
				if($row['is_admin'] == 1)
				{
					...
					if(isset($_GET['product']))
					{
						...
						$delete_product_images_query = "DELETE 
													FROM ecom_product_image 
													WHERE product_id IN (SELECT id 
													FROM ecom_product 
													WHERE id = '$product_id')";
						// run delete product images query
						$delete_product_images_result = mysqli_query($connection, $delete_product_images_query);
						...
						$delete_product_comments_result = mysqli_query($connection, $delete_product_comments_query);
					...
					if(isset($_GET['user']) AND $_GET['user'] == $_SESSION['e_commerce_username'])
					{
					...
					elseif(isset($_GET['user'])) 
					{
						...
						$delete_user_post_result = mysqli_query($connection, $delete_user_post_query);						

						// DELETE USER
						// create delete user query
						$delete_user_query = "DELETE FROM ecom_user
												WHERE username = '$username'";
					...
					if(isset($_GET['comment']))
					{
						...
						$delete_comment_query = "DELETE FROM ecom_post
												WHERE id = $comment_id";
