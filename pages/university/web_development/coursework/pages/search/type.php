	...
	if (isset($_GET['type']))
		{
			...
			$description_query = "SELECT description 
	         					FROM wd_type
	         					WHERE wd_type.name = '".$type."'
	         					LIMIT 1";

			// main query - all products from specific product type
			$query = "SELECT wd_product.id AS product_id, wd_product.name AS product_name, wd_product.manufacturer_id AS manufacturer_id,year, price, wd_product.description AS product_description, manufacturer, image, wd_type.name AS type_name
					FROM wd_product
