<?php
	...
	if (isset($_GET['manufacturer']))
		{
			...
			$description_query = "SELECT *
								FROM wd_manufacturer
	         					WHERE wd_manufacturer.manufacturer = '".$manufacturer."'
	         					LIMIT 1";								
			...
			$query = "SELECT wd_product.id AS product_id, wd_product.name AS product_name, price, year, wd_product.description AS product_description, wd_product.manufacturer_id AS manufacturer_id, manufacturer, image, wd_type.name AS type_name 
					FROM wd_product
	...
	// if the page has NOT been passed a specific type variable then do the following
		...
		{
			$query = "SELECT * FROM wd_type ORDER BY name";
