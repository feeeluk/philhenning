		...
		if (mysqli_num_rows($result) > 0)
		{
			while($row = mysqli_fetch_array($result, MYSQL_ASSOC))
			{
				if($row['is_admin'] == 1)
				{
					...
					$product_query = "SELECT wd_product.name AS product_name, wd_manufacturer.id AS manufacturer_id, wd_manufacturer.manufacturer AS manufacturer, wd_type.id AS type_id, wd_type.name AS type_name, price, year, wd_product.description AS description
									FROM wd_product, wd_manufacturer, wd_type
									WHERE wd_product.id = '$product_id'
									AND wd_product.manufacturer_id = wd_manufacturer.id
									AND wd_product.type_id = wd_type.id";
					...
					if(isset($_GET['delete']))
						{
							...
							// delete image query
							$delete_query = "DELETE FROM wd_product_image
											WHERE  id = '".$image_id."'";
					...
					if(isset($_POST['submit']))
					{
						...
						if ($not_empty_name == true AND $not_empty_manufacturer == true AND $not_empty_type == true AND $not_empty_price == true AND $not_empty_year == true AND $not_empty_description == true)
						{
							...
							// insert update query
							$query = 
									"UPDATE wd_product AS product
									SET
									name = '$name',
									product.manufacturer_id = (
									   SELECT manufacturer.id
									   FROM wd_manufacturer AS manufacturer
									   WHERE manufacturer.manufacturer = '$selected_manufacturer'
									),
									product.type_id = (
									   SELECT type.id
									   FROM wd_type AS type
					...
					$image_query = "SELECT name, location, wd_product_image.id AS image_id
