		...
		if (mysqli_num_rows($result) > 0)
		{
			while($row = mysqli_fetch_array($result, MYSQL_ASSOC))
			{
				if($row['is_admin'] == 1)
				{
					...
					if(isset($_GET['user']) AND $_GET['user'] == $_SESSION['web_development_username'])
					{
					...
					elseif(isset($_GET['user'])) 
					{
						...
						$delete_user_query = "DELETE FROM wd_user
												WHERE username = '$username'";
					...
					if(isset($_GET['comment']))
					{
						...
						$delete_comment_query = "DELETE FROM wd_post
												WHERE id = $comment_id";
