...
if(isset($_POST['submit']))
	...
	// assign form input values to the corresponding variables. 'selected' variables are used to store the currently selected option of drop down fields, so that the selected option will be maintained when the form is refreshed
		...
		$query = "SELECT id FROM wd_product WHERE id = '$product_id'";
		...
		if ($not_empty_product_id == true AND $new_product_id == true AND $not_empty_name == true AND $not_empty_manufacturer == true AND $not_empty_type == true AND $not_empty_price == true AND $not_empty_year == true AND $valid_year == true AND $not_empty_description == true)
		{
			...
			// insert update query
			$query = "INSERT INTO wd_product (id, name, manufacturer_id, type_id, price, year, description)
					SELECT '$product_id' AS product_id, '$name' AS name, wd_manufacturer.id AS manufacturer_id, wd_type.id AS type_id, '$price' AS price, '$year' AS year, '$description' AS description
					FROM wd_manufacturer, wd_type
					WHERE wd_manufacturer.manufacturer = '$selected_manufacturer' AND wd_type.name = '$selected_type'";
			
			mysqli_query($connection, $query) or die(mysqli_error($connection)); or die(mysqli_error($connection));
			...
			// UPLOAD PRODUCT IMAGES TO SERVER
				...
				function upload_image($x)
				{
					...
					$insert_query = "INSERT INTO wd_product_image (product_id, location)
	...
	<form id="add" action="add_product.php" method="post" enctype="multipart/form-data">
		...
		<span class="xxs">Manufacturer</span>
			<select name="manufacturer" id="manufacturer">
				<?php
					...
					$query = "SELECT id, manufacturer FROM wd_manufacturer";
					$result = mysqli_query($connection, $query) or die(mysqli_error($connection));;
					if(mysqli_num_rows($result) > 0)
					{
		...
		<span class="xxs">Type</span>
			<select name="type" id="type">
				<?php
					// pull the values for the drop down menu out of the corresponding database table
					$query = "SELECT id, name FROM wd_type";
					$result = mysqli_query($connection, $query) or die(mysqli_error($connection));;
					if(mysqli_num_rows($result) > 0)
					{
						// Set the default drop down option unless already selected
